  ...
  return (
    <header className="flex justify-between items-center h-20 bg-white px-8 border-b border-gray-300 shadow-sm">
      ...
      <nav className="hidden md:flex items-center gap-8">
        ...
        {!isLoginPage && (
          <Link to="/login" className="text-1xl font-semibold text-white bg-[#FE623F] px-5 py-2.5 rounded-[15px]">
            LOG IN
          </Link>
        )}
      </nav>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 top-[80px] bg-white z-50 md:hidden">
          <nav className="flex flex-col items-center gap-8 pt-8">
            ...
            {!isLoginPage && (
              <Link to="/login" className="text-xl font-semibold text-white bg-[#FE623F] px-5 py-2.5 rounded-[15px]" onClick={() => setIsMenuOpen(false)}>
                LOG IN
              </Link>
            )}
          </nav>
