...
const ApplicationSummary: FC = () => {
  ...
  const [formData, setFormData] = useState<ApplicationSummary | null>(null);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="space-y-2">
                      <p><span className="text-gray-500">Establishment:</span> {formData.establishmentName}</p>
                      <p><span className="text-gray-500">Owner:</span> {formData.ownerName}</p>
                      {formData.representativeName && (
                        <p><span className="text-gray-500">Representative:</span> {formData.representativeName}</p>
                      )}
                      {formData.tradeName && (
                        <p><span className="text-gray-500">Trade Name:</span> {formData.tradeName}</p>
                      )}
                      <p><span className="text-gray-500">Occupancy Type:</span> {formData.occupancyType}</p>
                      <p><span className="text-gray-500">Floor Area:</span> {formData.totalFloorArea} m²</p>
                      <p><span className="text-gray-500">No. of Storeys:</span> {formData.numberOfStoreys}</p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      <p><span className="text-gray-500">Address:</span> {formData.streetName}</p>
                      <p><span className="text-gray-500">Barangay:</span> {formData.barangay}</p>
                      ...
                      {formData.landline && (
                        <p><span className="text-gray-500">Landline:</span> {formData.landline}</p>
                      )}
                      {formData.contactNumber && (
                        <p><span className="text-gray-500">Mobile:</span> {formData.contactNumber}</p>
                      )}
                    </div>
                  </div>
                </div>
